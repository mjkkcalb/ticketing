...
| 메인 | - 서울시 공공 문화 데이터 출력
- 카테고리 별 필터 적용
- 현재 날짜 기준 진행예정, 진행중, 진행 종료 확인
- 검색 기능
- 예약 가능, 종료 이벤트 체크 시 해당 리스트 출력 |
| 장바구니 | - 저장된 장바구니 데이터 출력
- 삭제 버튼
- 결제 버튼 클릭 시 결제창 이동 
- 결제 시 장바구니 데이터 자동 삭제 |
| 결제페이지 | - mongoDB에 저장된 해당 데이터 출력
- 필수 작성칸(이름, 휴대전화, 이메일) 미입력 시 경고 알림
- 결제 방식 선택
- 결제 버튼 클릭 시 결제 시간 및 결제 방식 저장 |
| 마이페이지 | - 프로필 이미지 선택 및 수정
- 로그아웃
- 결제 내역 확인
- 예약 내역 확인 
- 내가 작성한 리뷰 확인(미작업)  |
